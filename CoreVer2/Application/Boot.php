...
class Boot
{
    ...
    public function __construct()
    {
        $this->ver1Legacy();
        /*
         * // Lấy config $configGlobal = Config\Config::system('Global');
         ...
         * Config\Configure::set('ConfigDatabase',
         * Config\Config::system('Database')); // Các đối tượng trợ giúp
         * $this->_router = new Network\Router(); $this->_request = new
         * Network\Request();
         * $this->_router->setting($configGlobal['app']['use'],
         * $configGlobal['language']['multi'],
         * $configGlobal['language']['position'],
         * $configGlobal['app']['default'], $configGlobal['locale']['lang'])
         * ->homeRoute($configApp['indexAppInfo'])
         * ->webPath($configGlobal['site']['path']);
         * $this->_request->addRouter($this->_router);
         * $this->_request->process(); echo $configGlobal['site']['path'] . ' --
         * ' . $this->_request->responeCode; var_dump($this->_request->app);
         */
    }
