...
class Validator extends Vhmis\Validator\ValidatorAbstract
{
    ...
    /**
     * Validator này sử dụng chung, dùng để kiểm tra nhiều thứ, nên hàm isValid không được sử dụng
    ...
    public function isValid($value, $option = null)
    {
        ...
        if (!isset($option['type']) || $option['type'] !== 'NotEmpty'
                || $option['type'] !== 'InRange' || $option['type'] !== 'Regex') {
