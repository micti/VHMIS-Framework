...
class Vhmis_Component_Acl
{
    ...
    {
        if(!isset($this->_apps[$app])) $this->_apps[$app] = array();
    }
    ...
    {
        $this->addApp($app);
        if(!isset($this->_apps[$app][$resource])) $this->_apps[$app][$resource] = array();
    }

    public function addGroup($group)
    {
        if(in_array($group, $this->_groups)) return;
    ...
    {
        if(in_array($department, $this->_departments)) return;
    ...
    {
        if(in_array($user, $this->_users)) return;
    ...
    public function isAllow($user, $app, $resource, $action)
    {
        ...
        if(isset($this->_acl[$app][$resource]['_all']['u_' . $user['id']]) && $this->_acl[$app][$resource]['_all']['u_' . $user['id']] < 2) return $this->_acl[$app][$resource]['_all']['u_' . $user['id']];
        ...
        if($user['groups'] !== null)
        {
            foreach($user['groups'] as $group)
            {
                ...
                if(isset($this->_acl[$app][$resource][$action]['g_' . $group]) && $this->_acl[$app][$resource][$action]['g_' . $group] == 1) return 1;
    ...
    public static function makeAclFromDbResult($permissions)
    {
        ...
        if($permissions !== null)
        {
            ...
            {
                if($permission->group_id != 0) $acl[$permission->app][$permission->resource][$permission->action]['g_' . $permission->group_id] = $permission->privileges;

                if($permission->user_id != 0) $acl[$permission->app][$permission->resource][$permission->action]['u_' . $permission->user_id] = $permission->privileges;

                if($permission->department_id != 0) $acl[$permission->app][$permission->resource][$permission->action]['d_' . $permission->department_id] = $permission->privileges;
            }
        }
