...
class Vhmis_Component_Calendar extends Vhmis_Component
{
    ...
    public function findEvents($start, $end, $user, $returnType)
    {
        ...
        foreach($events as $event)
        {
            ...
            $invite = array();
            if($event['invite_name_department'] != '') $invite[] = $event['invite_name_department'];
            if($event['invite_name_group'] != '') $invite[] = $event['invite_name_group'];
            if($event['invite_name_individual'] != '') $invite[] = $event['invite_name_individual'];
            ...
            $timeHourRange = current(explode(':', $time)) . ':00'; // Mốc giờ bắt đầu
            $date = $date[0];
            $related = false;
            ...
            if($event['type'] == 0 || $event['type'] == 1 || $event['type'] == 2) $related = true;
            else
            {
                if(strpos(',' . $event['invite_not_individual'] . ',', ',' . $user['id'] . ',') === false)
                {
                    ...
                    else if(strpos(',' . $event['invite_individual'] . ',', ',' . $user['id'] . ',') !== false)
                    {
                        $related = true;
                    }
                    else
            ...
            if($event['repeat'] == 1)
            {
                // Nếu sự kiện gốc còn nằm trong khoảng thì cho vào sự kiện gốc vào danh mục sự kiện
    ...
    public function findRepeatedDate($baseDate, $startDate, $endDate, $event)
    {
        ...
        {
            $repeatedDates = $dateRepeat->calculateWeeklyRepeat(array(
        ...
        {
            $repeatedDates = $dateRepeat->calculateMonthlyRepeat(array(
                'freq' => $event['repeat_freq'],
        ...
        {
            $repeatedDates = $dateRepeat->calculateYearlyRepeat(array(
                'freq' => $event['repeat_freq'],
                'type' => $event['repeat_type'],
