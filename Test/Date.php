...
var_dump(date("e", time()));
var_dump(date("I", time()));
var_dump(date("O", time()));
var_dump(date("P", time()));
var_dump(date("T", time()));
var_dump(date("Z", time()));
...
$date = new Vhmis_Date();
$mark->timer('b');
echo $mark->time('a', 'b') . '<br />';

echo '<br>';
echo $date . '|';
echo $date->toSQL() . '|';
echo $date->toSQL(false) . '|';
echo var_dump($date);

$mark->timer('c');
echo '<br />' . $mark->time('a', 'c') . '<br />';
...
$mark->timer('d');
echo '<br />' . '<br />' . $mark->time('a', 'd') . '<br />';

$date->time('-1 day -3 hour');
echo '<br>';
echo $date->toAgo();
