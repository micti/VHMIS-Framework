...
class Vhmis_Date_Repeat
{
    ...
    public function calculateWeeklyRepeat($rule)
    {
        ...
        while ($this->_runDate->differentDay($this->_endDate) >= 0) {
            ...
            foreach ($rule['weekday'] as $weekday) {
                ...
                if ($time < $this->_nowDate->getUnixTime() || $time > $this->_endDate->getUnixTime())
                    continue;
     ...
     * 'type' => Loại lặp lại (lặp theo ngày trong tháng, hoặc lặp theo ngày
     * trong tuần)
     * 'weekday' => Thứ ngày xáy ra sự kiện trong tuần của sự kiện
     * 'weekday_position' => Vị trí tuần xảy ra sự kiện (0 là mỗi tuần, 5 cuối
        ...
        while ($this->_runDate->differentDay($this->_endDate) >= 0) {
            foreach ($rule['month'] as $month) {
                ...
                foreach ($rule['monthday'] as $day) {
                    ...
                    // Vẫn có trường hợp thời gian xảy ra trước thời gian hiện
                    // tại và sau thời gian dừng
                    if ($time < $this->_nowDate->getUnixTime() || $time > $this->_endDate->getUnixTime())
                        continue;
